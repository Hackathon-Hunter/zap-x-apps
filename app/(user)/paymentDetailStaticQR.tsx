...
const PaymentDetailStaticQR: React.FC = () => {
  ...
  const qrData = React.useMemo(
    () => ({
      merchant: (params.merchant ?? 'IDRX Money Changer') as string,
      currency: (params.currency ?? 'IDR') as string,
      adminFee: (params.adminFee ?? '1000') as string,
    }),
    [params.merchant, params.currency, params.adminFee]
  );
  ...
  React.useEffect(() => {
    ...
    setCurrency(SUPPORTED_CURRENCIES[index] || SUPPORTED_CURRENCIES[0]);
  }, [qrData]);
  ...
  const calculateTokenNeeded = () => {
    if (!selectedToken || !amount || !currency) return '0';

    const totalAmount =
      parseFloat(amount.replace(/,/g, '')) +
      parseFloat(qrData.adminFee.replace(/,/g, '') || '0');
  ...
  const getTotalAmount = () => {
    ...
    const adminFeeNum = parseFloat(qrData.adminFee.replace(/,/g, '') || '0');
    return (amountNum + adminFeeNum).toString();
  };
      ...
      <ScrollView className="flex-1 bg-black">
        <View className="flex-1 px-4 py-6">
          ...
          <View className="bg-black border-gray-700 border-[0.5px] p-4 mb-6">
            ...
            <DetailRow label="Currency" value={currency?.name || 'IDR'} />
            <DetailRow
              label="Admin Fee"
          ...
          {amount && (
            <View className="mb-4 bg-black border-gray-700 border-[0.5px] p-4">
              <DetailRow
                label="Amount"
                prefix={currency?.symbol}
                value={currencyFormatter(Number(amount.replace(/,/g, '')))}
              />
              <DetailRow
                label="Admin Fee"
                prefix={currency?.symbol}
              ...
              <DetailRow
                label="Total"
                prefix={currency?.symbol}
                value={currencyFormatter(Number(getTotalAmount()))}
                isTotal={true}
              />
            </View>
          )}
          ...
          >
            This will convert {selectedToken?.symbol} to {currency?.symbol} and
            send to merchant
          </ThemedText>
...
export default PaymentDetailStaticQR;
